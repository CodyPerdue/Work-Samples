    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or love.keyboard.wasPressed('space') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
				...
				local tempTile = self.highlightedTile
                local newTile = self.board.tiles[y][x]
				
				self.board:swap(tempTile, newTile)

                -- tween coordinates between the two so they swap
                Timer.tween(0.1, {
                    [tempTile] = {x = newTile.x, y = newTile.y},
                    [newTile] = {x = tempTile.x, y = tempTile.y}
                })
                
                -- once the swap is finished, we can tween falling blocks as needed
				-- if the match failed, swap the blocks back to their original positions
                :finish(function()
                    if not self:calculateMatches() then
						...
						-- once movement is done and matches are calculated, test if board has matches
						-- reset if none available
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- add score and increment timer for each match
...
--[[
	Resets the board, includes a visual display
]]
