...
function LevelMaker.createMap(level)
	...
	-- whether we can spawn a locked brick, and if we have
	local lockedFlag = level % 5 == 0 and true or false
	local lockedSpawned = false
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
			...
			-- random chance to replace brick with locked brick if level interval of 5 and not spawned yet
			local r = math.random(1, 10)	-- 10% chance
			if r == 1 and lockedFlag == true and lockedSpawned == false then
				b.color = 6
				b.tier = 3
				b.locked = true
				lockedSpawned = true
