...
function Level:init()
	...
	-- alien splitting variables
	self.playerCollided = false
	self.clonesSpawned = false
	self.leftClone = nil
	self.rightClone = nil
...
function Level:update(dt)
	...
	-- add clones if space is pressed and player has not collided
	if love.keyboard.wasPressed('space') and self.clonesSpawned == false and self.playerCollided == false then
		-- grab data from original player alien
		local origAlien = self.launchMarker.alien

		-- spawn left clone
		self.leftClone = Alien(self.world, 'round', origAlien.body:getX(), origAlien.body:getY() - 35, 'Player')

		self.leftClone.body:setLinearVelocity(origAlien.body:getLinearVelocity())

		self.leftClone.fixture:setRestitution(0.4)
		self.leftClone.body:setAngularDamping(1)

		-- spawn right clone
		self.rightClone = Alien(self.world, 'round', origAlien.body:getX(), origAlien.body:getY() + 35, 'Player')

		self.rightClone.body:setLinearVelocity(origAlien.body:getLinearVelocity())

		self.rightClone.fixture:setRestitution(0.4)
		self.rightClone.body:setAngularDamping(1)

		-- add to players table
		table.insert(self.players, self.leftClone)
		table.insert(self.players, self.rightClone)
    ...
    if self.launchMarker.launched then
        ...
        if resetReady then
			if self.clonesSpawned == true then
				-- destroy alien bodies
				self.leftClone.body:destroy()
				self.rightClone.body:destroy()

				-- remove all clones from player table
				for i = #self.players, 1, -1 do
					table.remove(self.players, i)
				end
			end

			-- reset cloning variables
			self.playerCollided = false
			self.clonesSpawned = false
			self.leftClone = nil
			self.rightClone = nil

			-- respawn alien to launch
